...
face_antispoofing_postprocess::face_antispoofing_postprocess() {
...
*/
HIAI_StatusT face_antispoofing_postprocess::Init(
...
bool face_antispoofing_postprocess::IsInValidIp(const std::string &ip) {
  regex re(kIpRegularExpression);
  smatch sm;
  return !regex_match(ip, sm, re);
}

bool face_antispoofing_postprocess::IsInValidPort(int32_t port) {
  return (port <= kPortMinNumber) || (port > kPortMaxNumber);
}

bool face_antispoofing_postprocess::IsInValidChannelName(
...
bool face_antispoofing_postprocess::IsInvalidConfidence(float confidence) {
  return (confidence <= kConfidenceMin) || (confidence > kConfidenceMax);
}

bool face_antispoofing_postprocess::IsInvalidResults(float attr, float score,
...
bool face_antispoofing_postprocess::IsSupportFormat(hiai::IMAGEFORMAT format) {
  return format == hiai::YUV420SP;
}

HIAI_StatusT face_antispoofing_postprocess::ConvertImage(hiai::ImageData<u_int8_t>& org_img) {
...
HIAI_StatusT face_antispoofing_postprocess::HandleResults(
...
HIAI_IMPL_ENGINE_PROCESS("face_antispoofing_postprocess", face_antispoofing_postprocess,INPUT_SIZE)
{
    if (arg0 == nullptr) {
    HIAI_ENGINE_LOG(HIAI_ENGINE_RUN_ARGS_NOT_RIGHT,
